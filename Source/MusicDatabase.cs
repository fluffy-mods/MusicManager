...
namespace MusicManager
{
    public class SongDatabase: IExposable
    {
        ...
        public SongMetaData this[SongDef song]{
            get{
                if (_database.TryGetValue(song, out var meta ) )
                    return meta;
        ...
        public void Initialize()
        {
            ...
            foreach ( var song in DefDatabase<SongDef>.AllDefsListForReading )
            {
                if ( _database.ContainsKey( song ) ) continue;
        ...
        {
            var pre = _database.Count;
            var _custom = _database.Where( ( p ) => p.Value.HasCustomMetaData ).Select( p => p.Value ).ToList();
        ...
        public bool HasCustomMetaData {
            get {
                ...
                if (time != original.time )
                    return true;
                if ( seasons?.Count != original.seasons?.Count )
                    return true;
                if ( seasons == null && original.seasons == null )
                    return false;
                if ( seasons.Except( original.seasons ).Any() || original.seasons.Except( seasons ).Any() )
                    return true;
                return false;
            }
        }
